...
describe("StakeRoot cluster — upgrade migration", function () {
  ...
  it("BroadsideStakeRoot copies reporters + threshold + finalized roots", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideStakeRoot")).deploy();
  ...
  it("BroadsideStakeRootV2 copies stakes + commitments + payouts and sweeps DOT", async function () {
    const tok = treasury.address; // datumToken not exercised here
    const v1 = await (await ethers.getContractFactory("BroadsideStakeRootV2"))
  ...
  it("BroadsideBondedIdentityReporter copies reporter stakes + nonces and sweeps DOT", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideBondedIdentityReporter"))
