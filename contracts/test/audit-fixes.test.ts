...
describe("Audit fixes", function () {
  ...
  describe("M-1: ChallengeBonds.returnBond DoS-proof for contract advertiser", function () {
    let bonds: BroadsideChallengeBonds;
    let rejecting: MockRejectingReceiver;
    let campaignsMock: HardhatEthersSigner;
    let lifecycleMock: HardhatEthersSigner;
    ...
    before(async function () {
      ...
      const BondsFactory = await ethers.getContractFactory("BroadsideChallengeBonds");
  ...
  describe("M-3 + dual-sig toggle", function () {
    ...
    let dualSig: any; // BroadsideDualSig (alpha-4 EIP-170 carve-out)
    let validator: BroadsideClaimValidator;
    ...
    before(async function () {
      ...
      const DualSigF = await ethers.getContractFactory("BroadsideDualSig");
  ...
  describe("G-M1: Router admin* shortcuts gated on Admin phase", function () {
    it("rejects adminActivateCampaign when phase != Admin", async function () {
      ...
      const RouterFactory = await ethers.getContractFactory("BroadsideRouter");
  ...
  describe("G-M5/G-M6: PublisherGovernance propose bond + treasury sweep", function () {
    ...
    before(async function () {
      ...
      const StakeFactory = await ethers.getContractFactory("BroadsidePublisherStake");
      ...
      const PgFactory = await ethers.getContractFactory("BroadsidePublisherGovernance");
  ...
  describe("R-M1: BroadsideZKVerifier.setVerifyingKey is once-only", function () {
    it("second setVerifyingKey reverts E01", async function () {
      const Factory = await ethers.getContractFactory("BroadsideZKVerifier");
  ...
  describe("R-L1: Publishers stakeGate freeze", function () {
    it("setStakeGate reverts after lockStakeGate", async function () {
      ...
      const PauseFactory = await ethers.getContractFactory("BroadsidePauseRegistry");
      const pause = await PauseFactory.deploy(signers[0].address, signers[1].address, signers[2].address);
      const PubFactory = await ethers.getContractFactory("BroadsidePublishers");
  ...
  describe("TM-1: AssuranceLevel raise locked once Active", function () {
    it("rejects setCampaignAssuranceLevel raise on an Active campaign", async function () {
      // Targets the live BroadsideCampaigns contract:
      await fundSigners();
      const signers = await ethers.getSigners();
      ...
      const PubsFactory = await ethers.getContractFactory("BroadsidePublishers");
    ...
    // ── A3: AssuranceLevel raise/lower semantics on real BroadsideCampaigns ───
  ...
  describe("TM-3: Council guardian/member mutual exclusivity", function () {
    it("rejects member added when they are already guardian", async function () {
      ...
      const Factory = await ethers.getContractFactory("BroadsideCouncil");
