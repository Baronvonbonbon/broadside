...
describe("Tag* — upgrade migration", function () {
  ...
  it("BroadsideTagSystem migrates the tag dictionary + campaign tag snapshots", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideTagSystem")).deploy();
  ...
  it("BroadsideTagRegistry migrates juror stakes + tags and sweeps DATUM", async function () {
    ...
    const v1 = await (await ethers.getContractFactory("BroadsideTagRegistry")).deploy(await datum.getAddress());
    await v1.setRouter(await router.getAddress());
