 ...
 *   BM-RPT-*  Community reports (inline in BroadsideCampaigns: page + ad reports)
 *   BM-TAG-*  Tag-based targeting (inline in BroadsideCampaigns)
...
import {
  ...
  BroadsideCampaignLifecycle,
  BroadsideAttestationVerifier,
  BroadsideClaimValidator,
  BroadsideTokenRewardVault,
  BroadsideSettlement,
  MockERC20,
...
describe("Datum Alpha-3 Benchmark Suite", function () {
  ...
  let reports:      any;
  let tagSystem:    any;
  let lifecycle:    BroadsideCampaignLifecycle;
  let v2:           BroadsideGovernance;
  let claimVal:     BroadsideClaimValidator;
  let settlement:   BroadsideSettlement;
  let relay:        BroadsideRelay;
  let verifier:     BroadsideAttestationVerifier;
  let tokenRewardVault: BroadsideTokenRewardVault;
  ...
  before(async function () {
    ...
    // 1. Infrastructure
    pauseReg  = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, advertiser.address, publisher.address);
    publishers = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(
      TAKE_RATE_DELAY, await pauseReg.getAddress()
    );
    ledger = await (await ethers.getContractFactory("BroadsideBudgetLedger")).deploy();
    vault  = await (await ethers.getContractFactory("BroadsidePaymentVault")).deploy();

    // 2. Campaigns (targeting + validation now inline)
    campaigns = await (await ethers.getContractFactory("BroadsideCampaigns")).deploy(
    ...
    lifecycle = await (await ethers.getContractFactory("BroadsideCampaignLifecycle")).deploy(
      await pauseReg.getAddress(), 432000n
    );
    v2 = await (await ethers.getContractFactory("BroadsideGovernance")).deploy(
    ...
    claimVal = await (await ethers.getContractFactory("BroadsideClaimValidator")).deploy(
      await campaigns.getAddress(), await publishers.getAddress(), await pauseReg.getAddress()
    );
    settlement = await (await ethers.getContractFactory("BroadsideSettlement")).deploy(
      await pauseReg.getAddress()
    );
    await wireSettlementLogic(settlement as any);
    relay = await (await ethers.getContractFactory("BroadsideRelay")).deploy(
      await settlement.getAddress(), await campaigns.getAddress(), await pauseReg.getAddress()
    );
    verifier = await (await ethers.getContractFactory("BroadsideAttestationVerifier")).deploy(
      await settlement.getAddress(), await campaigns.getAddress(), await pauseReg.getAddress()
    );

    // 6. BM-5: Rate limiter (carved out — BroadsideSettlementRateLimiter)
    const rateLimiter = await (await ethers.getContractFactory("BroadsideSettlementRateLimiter")).deploy();
    ...
    creative = await (await ethers.getContractFactory("BroadsideCampaignCreative")).deploy();
    await creative.setCampaigns(await campaigns.getAddress());
    await creative.setPauseRegistry(await pauseReg.getAddress());

    reports = await (await ethers.getContractFactory("BroadsideReports")).deploy();
    ...
    tokenRewardVault = await (await ethers.getContractFactory("BroadsideTokenRewardVault")).deploy(await campaigns.getAddress());
  ...
  // Stub verifier (BroadsideZKVerifier) accepts any proof with length > 0.
  ...
  // ---------------------------------------------------------------------------
  // BM-RPT: Community reports (inline in BroadsideCampaigns)
  // ---------------------------------------------------------------------------
  describe("BM-RPT: Community reports (inline in BroadsideCampaigns)", function () {
  ...
  // ---------------------------------------------------------------------------
  // BroadsideTokenRewardVault: advertiser deposits ERC-20 budget; settlement credits
