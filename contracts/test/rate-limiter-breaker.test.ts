...
import { BroadsideSettlementRateLimiter } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { mineBlocks } from "./helpers/mine";

const WINDOW = 10n; // rate-limit window, in blocks

// Aggregate (protocol-wide) circuit breaker on BroadsideSettlementRateLimiter.
// `settlement` is wired to a plain signer so we can drive tryConsume directly.
describe("BroadsideSettlementRateLimiter — aggregate circuit breaker", function () {
  let rl: BroadsideSettlementRateLimiter;
  ...
  beforeEach(async function () {
    ...
    rl = await (await ethers.getContractFactory("BroadsideSettlementRateLimiter")).deploy();
