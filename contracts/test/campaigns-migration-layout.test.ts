/// Storage-layout invariant for the BroadsideCampaigns migration DELEGATECALL.
///
/// BroadsideCampaigns.migrateDelegate() DELEGATECALLs BroadsideCampaignsMigrationLogic,
/// which keeps the caller's storage context. The Logic therefore MUST declare
/// the exact same storage layout as BroadsideCampaigns or its writes land in the
/// wrong slots and corrupt campaign state.
///
/// Unlike the Settlement split (which shares BroadsideSettlementStorage), the Logic
/// duplicates the layout: IBroadsideCampaigns names 8 of these fields as interface
/// getters, so a shared interface-free base would force BroadsideCampaigns to drop
...
/// between BroadsideCampaigns' storage and the Logic's copy fails here at CI time,
...
async function readStorageLayout(contractName: string): Promise<StorageLayout> {
  ...
  >;
  const entry = contracts[`src/${contractName}.sol`]?.[contractName];
...
describe("BroadsideCampaigns migration storage-layout invariant", function () {
  let layoutCampaigns: StorageLayout;
  let layoutLogic: StorageLayout;

  before(async function () {
    layoutCampaigns = await readStorageLayout("BroadsideCampaigns");
    layoutLogic = await readStorageLayout("BroadsideCampaignsMigrationLogic");
  });
  ...
  it("both contracts have the same number of storage slots", function () {
    expect(layoutLogic.storage.length).to.equal(
      ...
      "MigrationLogic slot count differs from BroadsideCampaigns -- a field was " +
        "added/removed/reordered on one but not the other."
    );
  });

  it("BroadsideCampaigns vs BroadsideCampaignsMigrationLogic: slot-by-slot identical", function () {
