...
// PRE-MAINNET-CHECKLIST §U3). BroadsidePublishers' registry is unbounded (100k+
...
describe("U3 — gas-paginated migration (BroadsidePublishers, unbounded registry)", function () {
  ...
  before(async function () {
    ...
    v1 = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(100, await pause.getAddress());
  ...
  it("rejects a mismatched predecessor mid-migration and non-governor callers", async function () {
    ...
    const other = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(100, await pause.getAddress());
