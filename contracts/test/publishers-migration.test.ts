...
// BroadsideUpgradable redeploy-migrate-rewire for the core publisher registry:
// freeze v1 → v2.migrate(v1) copies every registered publisher's record +
// per-publisher advertiser allowlist + registry config.
describe("BroadsidePublishers — upgrade migration", function () {
  ...
  beforeEach(async function () {
    ...
    pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, pub.address, adv.address);
