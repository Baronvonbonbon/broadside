...
describe("Settlement Nullifier (inline)", function () {
  ...
  let pauseRegistry: BroadsidePauseRegistry;
  ...
  before(async function () {
    ...
    pauseRegistry = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(
      owner.address, user.address, publisher.address
    ) as BroadsidePauseRegistry;

    mock = await (await ethers.getContractFactory("MockCampaigns")).deploy() as MockCampaigns;

    ledger = await (await ethers.getContractFactory("BroadsideBudgetLedger")).deploy() as BroadsideBudgetLedger;
    vault = await (await ethers.getContractFactory("BroadsidePaymentVault")).deploy() as BroadsidePaymentVault;

    validator = await (await ethers.getContractFactory("BroadsideClaimValidator")).deploy(
      await mock.getAddress(),
      await mock.getAddress(),
      await pauseRegistry.getAddress()
    ) as BroadsideClaimValidator;

    settlement = await (await ethers.getContractFactory("BroadsideSettlement")).deploy(
      await pauseRegistry.getAddress()
    ) as BroadsideSettlement;
    ...
    nullifiers = await (await ethers.getContractFactory("BroadsideNullifierRegistry")).deploy();
