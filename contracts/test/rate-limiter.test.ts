...
describe("Settlement Rate Limiter (inline)", function () {
  ...
  before(async function () {
    ...
    rateLimiter = await (await ethers.getContractFactory("BroadsideSettlementRateLimiter")).deploy();
  ...
  it("RL9: rate limiter disabled by default (rlWindowBlocks=0 initially)", async function () {
    ...
    const fresh = await (await ethers.getContractFactory("BroadsideSettlementRateLimiter")).deploy();
