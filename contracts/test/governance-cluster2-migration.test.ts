...
describe("Governance cluster 2 — GovernanceV2 / ParameterGovernance / Council migration", function () {
  ...
  beforeEach(async function () {
    ...
    pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, g3.address, g4.address);
  });

  it("BroadsideGovernance migrates in-flight conviction votes + sweeps locked DOT", async function () {
    ...
    const v1 = await (await ethers.getContractFactory("BroadsideGovernance")).deploy(...args);
  ...
  it("BroadsideParameterGovernance migrates whitelist + in-flight bonded votes", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideParameterGovernance")).deploy(await pause.getAddress(), 100, 50, parseDOT("1"), 0);
    await v1.setRouter(await router.getAddress());
  ...
  it("BroadsideCouncil migrates config; member set reconstructed at deploy", async function () {
    const guardian = owner.address; // not a member
    const members = [voter.address, g3.address, g4.address]; // MIN_COUNCIL_SIZE = 3
    const v1 = await (await ethers.getContractFactory("BroadsideCouncil")).deploy(members, 3, guardian, 100, 50, 30, 500);
    await v1.setRouter(await router.getAddress());
