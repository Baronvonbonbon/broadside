...
import { BroadsideStakeRootV2, MockERC20, MockIdentityVerifier } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { parseDOT } from "./helpers/dot";
import { mineBlocks, fundSigners } from "./helpers/mine";

// BroadsideStakeRootV2 — permissionless bonded reporter set + phantom-leaf
...
describe("BroadsideStakeRootV2", function () {
  let v2: BroadsideStakeRootV2;
  ...
  beforeEach(async function () {
    ...
    const F = await ethers.getContractFactory("BroadsideStakeRootV2");
    v2 = await F.deploy(
      treasury.address,
      MIN_STAKE,
      EXIT_DELAY,
  ...
  describe("CV: ClaimValidator dual stake-root wiring", function () {
    ...
    it("CV-2: setStakeRoot2 reverts after plumbingLocked", async function () {
      ...
      const Camps = await ethers.getContractFactory("BroadsideCampaigns");
