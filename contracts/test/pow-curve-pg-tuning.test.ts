...
//
// (Originally targeted BroadsideSettlement; PoW state + curve tuning moved
// to BroadsidePowEngine as part of the EIP-170 carve-out. Settlement no
...
describe("Stage 2: PowEngine + ClaimValidator PG dual-permission", function () {
  ...
  async function freshPowEngine() {
    return (await ethers.getContractFactory("BroadsidePowEngine")).deploy();
  }
  ...
  beforeEach(async function () {
    ...
    const Pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(
      owner.address, pgImpersonator.address, g2.address,
    );
    powEngine = await freshPowEngine();

    const Publishers = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(20n, await Pause.getAddress());
    const Campaigns = await (await ethers.getContractFactory("BroadsideCampaigns")).deploy(
  ...
  describe("Settlement-side setters unchanged", function () {
    it("RateLimiter.setRateLimits still owner-only (sanity)", async function () {
      const rl = await (await ethers.getContractFactory("BroadsideSettlementRateLimiter")).deploy();
