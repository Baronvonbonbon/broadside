// Path H BroadsideEmissionEngine tests — covers epoch state machine, daily cap
...
describe("BroadsideEmissionEngine (Path H)", function () {
  let engine: any;
  ...
  describe("setSettlement (phase-conditional lock-once)", function () {
    ...
    it("zero addr rejected", async function () {
      const fresh = await (await ethers.getContractFactory("BroadsideEmissionEngine")).deploy();
