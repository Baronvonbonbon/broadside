...
describe("Governance cluster — upgrade migration", function () {
  ...
  beforeEach(async function () {
    ...
    pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, g2.address, g3.address);
  });

  it("BroadsideRelayGovernance migrates config + in-flight vote state + sweeps locked DOT", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideRelayGovernance")).deploy(10, 100, 0, 5000, 2000, 1000); // proposeBond=0
  ...
  it("BroadsidePublisherGovernance copies config + sweeps DOT", async function () {
    const p = await pause.getAddress();
    const v1 = await (await ethers.getContractFactory("BroadsidePublisherGovernance")).deploy(p, p, p, 20, 4000, 500, 200, parseDOT("2"));
    await v1.setRouter(await router.getAddress());
    await v1.setConvictionLockups(LOCKS);
  ...
  it("BroadsideAdvertiserGovernance copies config", async function () {
    const p = await pause.getAddress();
    const v1 = await (await ethers.getContractFactory("BroadsideAdvertiserGovernance")).deploy(15, 3000, 150, parseDOT("1"), p);
    await v1.setRouter(await router.getAddress());
    await v1.setConvictionLockups(LOCKS);
