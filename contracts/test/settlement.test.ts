...
describe("BroadsideSettlement", function () {
  ...
  function buildClaimChain(
    ...
    for (let i = 1; i <= count; i++) {
      ...
      claims.push({
        ...
        rateWei: baseCpm,
        actionType: 0,
        proof: [],                 // SLIM (#2b): plain view claim — empty sidecar
        nonce,                     // JS-only bookkeeping (signing reads claims[i].nonce)
      });
      prevHash = hash;
    }
    return claims;
  ...
  before(async function () {
    ...
    const SettlementFactory = await ethers.getContractFactory("BroadsideSettlement");
    ...
    // alpha-4 EIP-170 carve-out: settleSignedClaims moved to BroadsideDualSig.
  ...
  // S4: CPM exceeding bidCpmWei is rejected
  it("S4: claim with clearingCpmWei > bidCpmWei is rejected", async function () {
    const cid = await createTestCampaign();
  ...
  // -----------------------------------------------------------------------
  // Publisher Relay (BroadsideRelay) — R1-R6, R7-R10
  // -----------------------------------------------------------------------

  describe("Publisher Relay (BroadsideRelay)", function () {
  ...
  describe("BM-10: min claim interval", function () {
    ...
    it("BM10-2: second settlement within interval rejects with reason code 18", async function () {
      ...
      const batch2 = {
        user: user.address,
        campaignId: cid,
        claims: [{ campaignId: cid, publisher: publisher.address, eventCount: 1000n, rateWei: BID_CPM, actionType: 0, proof: [] }],
      };
