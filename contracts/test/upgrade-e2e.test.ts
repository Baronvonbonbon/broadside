...
// stateful + fund-holding contract (BroadsidePublisherStake), with state and native
...
describe("Upgrade E2E — bump version with loaded data (BroadsidePublisherStake)", function () {
  ...
  beforeEach(async function () {
    ...
    v1 = await (await ethers.getContractFactory("BroadsidePublisherStake")).deploy(BASE, PER_IMP, DELAY);
...
// End-to-end validation of the BroadsideCampaigns carve-out upgrade: campaigns are
...
describe("Upgrade E2E — Campaigns carve-out, loaded via createCampaign", function () {
  ...
  beforeEach(async function () {
    ...
    publishers = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(50n, await pauseReg.getAddress());
    ledger = await (await ethers.getContractFactory("BroadsideBudgetLedger")).deploy();
    v1 = await (await ethers.getContractFactory("BroadsideCampaigns")).deploy(MIN_CPM, PENDING_TIMEOUT, await publishers.getAddress(), await pauseReg.getAddress());
  ...
  it("carries every campaign's full state (struct + pots + gates) across a version bump", async function () {
    ...
    await v2.setRouter(await router.getAddress());
    expect(await v2.version()).to.be.greaterThan(await v1.version());
    const logic = await (await ethers.getContractFactory("BroadsideCampaignsMigrationLogic")).deploy();
    const logicIface = (await ethers.getContractFactory("BroadsideCampaignsMigrationLogic")).interface;
    ...
    const ledger2 = await (await ethers.getContractFactory("BroadsideBudgetLedger")).deploy();
...
describe("Upgrade E2E — Governance vote-state (BroadsideRelayGovernance)", function () {
  ...
  beforeEach(async function () {
    ...
    router = await (await ethers.getContractFactory("MockOpenGovRouter")).deploy();
    await router.setGovernor(gov.address);
    v1 = await (await ethers.getContractFactory("BroadsideRelayGovernance")).deploy(10, 100, 0, 5000, 2000, 1000);
    await v1.setRouter(await router.getAddress());
...
// (BroadsidePublishers) underneath a live dependent (BroadsideCampaigns), migrate the
...
describe("Upgrade E2E — multi-contract rewire (Campaigns -> Publishers)", function () {
  ...
  beforeEach(async function () {
    ...
    await router.setGovernor(gov.address);
    pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, advertiser.address, pubA.address);

    pubsV1 = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(50n, await pause.getAddress());
    await pubsV1.setRouter(await router.getAddress());
    ledger = await (await ethers.getContractFactory("BroadsideBudgetLedger")).deploy();
    campaigns = await (await ethers.getContractFactory("BroadsideCampaigns")).deploy(MIN_CPM, PENDING_TIMEOUT, await pubsV1.getAddress(), await pause.getAddress());
