...
// BroadsideUpgradable redeploy-migrate-rewire for the fund-holding contracts:
...
describe("Fund-holder upgrade migration (BroadsideUpgradable)", function () {
  ...
  it("BroadsidePublisherStake: stake + impressions migrate; v2 solvent (unstake)", async function () {
  ...
  it("BroadsideAdvertiserStake: stake + budget-spent migrate; v2 solvent", async function () {
  ...
  it("BroadsideRelayStake: stake migrates with relayList + totalStaked + funds", async function () {
    const RS = await ethers.getContractFactory("BroadsideRelayStake");
  ...
  it("BroadsideZKStake: ERC-20 stake migrates; tokens swept to v2", async function () {
  ...
  it("BroadsideChallengeBonds: bond + pool + pending-return migrate; v2 solvent", async function () {
    const CB = await ethers.getContractFactory("BroadsideChallengeBonds");
  ...
  it("BroadsideActivationBonds: bond state + funds migrate", async function () {
    const AB = await ethers.getContractFactory("BroadsideActivationBonds");
  ...
  it("BroadsideTokenRewardVault: ERC-20 balances + budgets migrate; v2 solvent (withdraw)", async function () {
    ...
    await token.mint(a.address, parseDOT("100"));
    const TRV = await ethers.getContractFactory("BroadsideTokenRewardVault");
