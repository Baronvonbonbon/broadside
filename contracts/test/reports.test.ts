...
import { BroadsideCampaigns, BroadsidePublishers, BroadsidePauseRegistry, BroadsideBudgetLedger } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { parseDOT } from "./helpers/dot";
import { fundSigners } from "./helpers/mine";

// Community reporting — carved back out into BroadsideReports (alpha-4 EIP-170)
...
describe("BroadsideReports — community reporting", function () {
  ...
  before(async function () {
    ...
    // settlement stays unset; reporter-eligibility gate is skipped (test fixture)
  ...
  it("RP3: reportPage with reason=0 reverts E68", async function () {
    await expect(reports.connect(reporter1).reportPage(1, 0)).to.be.revertedWithCustomError(reports, "E68");
  });

  it("RP3b: reportPage with reason=6 reverts E68", async function () {
    await expect(reports.connect(reporter1).reportPage(1, 6)).to.be.revertedWithCustomError(reports, "E68");
  });
  ...
  it("RP6: reportAd with reason=6 reverts E68", async function () {
    await expect(reports.connect(reporter1).reportAd(1, 6)).to.be.revertedWithCustomError(reports, "E68");
  });

  it("RP6b: reportAd with reason=0 reverts E68", async function () {
    await expect(reports.connect(reporter1).reportAd(1, 0)).to.be.revertedWithCustomError(reports, "E68");
  });

  // RP7: non-existent campaign reverts E01
  it("RP7: reportPage on non-existent campaign reverts E01", async function () {
    await expect(reports.connect(reporter1).reportPage(999, 1)).to.be.revertedWithCustomError(reports, "E01");
  });

  it("RP7b: reportAd on non-existent campaign reverts E01", async function () {
    await expect(reports.connect(reporter1).reportAd(999, 1)).to.be.revertedWithCustomError(reports, "E01");
  });
