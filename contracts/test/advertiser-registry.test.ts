...
// Standalone advertiser-side registry — symmetric counterpart of BroadsidePublishers
// (relaySigner + profileHash + rotation cooldown). Staged for the next contract
// upgrade; replaces the in-BroadsideCampaigns advertiser additions to relieve EIP-170.
describe("BroadsideAdvertiserRegistry", function () {
  let registry: any;
  let pauseReg: any;
  ...
  beforeEach(async function () {
    ...
    const Registry = await ethers.getContractFactory("BroadsideAdvertiserRegistry");
  ...
  // Exercises the BroadsideUpgradable redeploy-migrate-rewire flow against the registry:
  // freeze v1 → deploy v2 → v2.migrate(v1) copies the enumerable advertiser state.
  describe("upgrade migration (BroadsideUpgradable)", function () {
    let router: any;
