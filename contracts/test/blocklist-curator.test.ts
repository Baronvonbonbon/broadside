...
import { BroadsideCouncilBlocklistCurator, BroadsidePublishers, BroadsidePauseRegistry } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { fundSigners } from "./helpers/mine";
import { wireOpenGovRouter } from "./helpers/openGovRouter";

// B2: Council-driven IBroadsideBlocklistCurator implementation.
// Verifies the curator can be wired into BroadsidePublishers and is consulted
// via publishers.isBlocked.

describe("BroadsideCouncilBlocklistCurator (B2)", function () {
  let curator: BroadsideCouncilBlocklistCurator;
  let publishers: BroadsidePublishers;
  let pauseReg: BroadsidePauseRegistry;

  let owner: HardhatEthersSigner;
  let councilEOA: HardhatEthersSigner;  // stand-in for BroadsideCouncil contract
  ...
  before(async function () {
    ...
    curator = await (await ethers.getContractFactory("BroadsideCouncilBlocklistCurator")).deploy();

    // Wire: curator's council = the EOA standing in for BroadsideCouncil
  ...
  it("BC7: lockCouncil with unset council reverts", async function () {
    const fresh = await (await ethers.getContractFactory("BroadsideCouncilBlocklistCurator")).deploy();
