...
import { BroadsideTagRegistry, MockERC20 } from "../typechain-types";
...
describe("BroadsideTagRegistry (StakeGated lane)", function () {
  let registry: BroadsideTagRegistry;
  let datum: MockERC20;

  let owner: HardhatEthersSigner;
  let campaigns: HardhatEthersSigner; // stand-in for BroadsideCampaigns address
  ...
  async function deployFresh() {
    ...
    registry = await (await ethers.getContractFactory("BroadsideTagRegistry"))
      .deploy(await datum.getAddress()) as unknown as BroadsideTagRegistry;
  ...
  describe("Construction & initial parameters", function () {
    ...
    it("rejects zero address for staking token", async function () {
      await expect(
        (await ethers.getContractFactory("BroadsideTagRegistry")).deploy(ethers.ZeroAddress),
