// Phase-A parameter-governance tests for BroadsideCampaigns and
...
async function deployCampaigns(initialFloor: bigint = parseDOT("0.001")) {
  ...
  const pauseReg = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(
    g0.address, g1.address, g2.address,
  );
  const publishers = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(
...
async function deployLifecycle() {
  ...
  const lifecycle = await (await ethers.getContractFactory("BroadsideCampaignLifecycle")).deploy(
    await pauseReg.getAddress(),
...
describe("Phase A — BroadsideCampaigns parameter governance", () => {
...
describe("Phase A — BroadsideCampaignLifecycle parameter governance", () => {
