...
// G-10 first close: ParameterRetuneGuard integration on BroadsideRelayGovernance.
...
describe("BroadsideRelayGovernance G-10 first close (ParameterRetuneGuard)", function () {
  let gov: BroadsideRelayGovernance;
  let stake: BroadsideRelayStake;
  ...
  beforeEach(async function () {
    ...
    const StakeFactory = await ethers.getContractFactory("BroadsideRelayStake");
    stake = await StakeFactory.deploy(0, 20n);

    const GovFactory = await ethers.getContractFactory("BroadsideRelayGovernance");
