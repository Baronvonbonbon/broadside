...
describe("Config-only _migrate (MintCoordinator, PowEngine)", function () {
  ...
  it("BroadsideMintCoordinator copies mint-rate + reward-split scalars", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideMintCoordinator")).deploy();
  ...
  it("BroadsidePowEngine copies the curve config (buckets reset by design)", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsidePowEngine")).deploy();
    await v1.setRouter(await router.getAddress());
