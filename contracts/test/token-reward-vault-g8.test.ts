...
import { BroadsideTokenRewardVault, MockCampaigns, MockERC20 } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";
import { mineBlocks, fundSigners } from "./helpers/mine";

// G-8 mirror: time-locked recovery address on BroadsideTokenRewardVault.
// Structural mirror of payment-vault-g8.test.ts adapted for per-token balances.

describe("BroadsideTokenRewardVault G-8 mirror (recovery address)", function () {
  let vault: BroadsideTokenRewardVault;
  ...
  beforeEach(async function () {
    ...
    const VaultFactory = await ethers.getContractFactory("BroadsideTokenRewardVault");
