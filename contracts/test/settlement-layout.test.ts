...
/// (phase 8d-5).
///
/// The two-Logic split (`BroadsideSettlement` -> `BroadsideSettlementLogicA` ->
/// `BroadsideSettlementLogicB`) relies on every participating contract
...
///
/// All three inherit `BroadsideSettlementStorage` (the single source of
 ...
 * `t_contract(IBroadsideRouter_Upgradable)58410` → `t_contract(IBroadsideRouter_Upgradable)`)
...
describe("Settlement storage layout invariant (phase 8d-5)", function () {
  ...
  before(async function () {
    layoutSettlement = await readStorageLayout("BroadsideSettlement");
    layoutLogicA = await readStorageLayout("BroadsideSettlementLogicA");
    layoutLogicB = await readStorageLayout("BroadsideSettlementLogicB");
  });
  ...
  it("BroadsideSettlement vs BroadsideSettlementLogicA: slot-by-slot identical", function () {
  ...
  it("BroadsideSettlement vs BroadsideSettlementLogicB: slot-by-slot identical", function () {
    const a = layoutSettlement.storage.map(normalizeSlot);
  ...
  it("BroadsideSettlementLogicA vs BroadsideSettlementLogicB: slot-by-slot identical", function () {
  ...
  it("current layout matches settlement-layout.snapshot.json (committed)", function () {
    ...
    // where BroadsideSettlementStorage is intentionally modified (adding a
