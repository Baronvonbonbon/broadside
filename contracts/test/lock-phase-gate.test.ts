...
//
// Confirms the whenOpenGovPhase modifier on BroadsideUpgradable:
...
//
// Uses BroadsideTagCurator as a representative contract (small + has
...
describe("Stage 4: whenOpenGovPhase on lock functions", function () {
  let router: BroadsideRouter;
  let curator: BroadsideTagCurator;
  ...
  beforeEach(async () => {
    ...
    const RouterF = await ethers.getContractFactory("BroadsideRouter");
    ...
    const CuratorF = await ethers.getContractFactory("BroadsideTagCurator");
