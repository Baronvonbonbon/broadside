...
//
// Authority model: owner OR ParameterGovernance OR BroadsideCouncil (emergency).
...
describe("Feature switches: DATUM emission + ERC sidecar", function () {
  ...
  // ── DATUM emission (BroadsideMintCoordinator) ─────────────────────────────────
  describe("BroadsideMintCoordinator.emissionEnabled", function () {
    let coordinator: any, engine: any;

    beforeEach(async function () {
      coordinator = await (await ethers.getContractFactory("BroadsideMintCoordinator")).deploy();
      engine = await (await ethers.getContractFactory("BroadsideEmissionEngine")).deploy();
      ...
      // the path we want, without standing up the full BroadsideMintAuthority stack.
  ...
  // ── ERC sidecar (BroadsideTokenRewardVault) ───────────────────────────────────
  describe("BroadsideTokenRewardVault sidecar switches", function () {
    ...
    beforeEach(async function () {
      ...
      vault = await (await ethers.getContractFactory("BroadsideTokenRewardVault")).deploy(await mock.getAddress());
  ...
  // ── Emission switch on the engine (BroadsideEmissionEngine) ───────────────────
  ...
  describe("BroadsideEmissionEngine.emissionEnabled", function () {
    let engine: any;
  ...
  describe("BroadsideTokenRewardVault asset gating", function () {
    let vault: any, mock: any, token: any, token2: any;
    const CID = 1n;
    const AMT = ethers.parseEther("1000");
    const TEN = ethers.parseEther("10");
  ...
  describe("BroadsideTokenRewardVault native-asset precompile", function () {
