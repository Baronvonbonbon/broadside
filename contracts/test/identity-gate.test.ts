...
//
// Covers:
//   ID1 — BroadsidePeopleChainIdentity unit: writer auth, expiry, batch, forgetMe.
...
describe("People Chain identity gate", function () {
  ...
  describe("BroadsidePeopleChainIdentity (ID1)", function () {
    let reg: BroadsidePeopleChainIdentity;
    ...
    beforeEach(async function () {
      ...
      const F = await ethers.getContractFactory("BroadsidePeopleChainIdentity");
      reg = await F.deploy();
  ...
  describe("Settlement gate (ID2–ID7)", function () {
    ...
    before(async function () {
      ...
      validator = await (await ethers.getContractFactory("BroadsideClaimValidator")).deploy(
        await mock.getAddress(),
        await mock.getAddress(),
        await pauseReg.getAddress()
      );
      settlement = await (await ethers.getContractFactory("BroadsideSettlement"))
      ...
      reg = await (await ethers.getContractFactory("BroadsidePeopleChainIdentity")).deploy();
      await reg.connect(owner).setOracleReporter(oracle.address);
    ...
    it("ID7: identity registry unwired but gate active fails CLOSED", async function () {
      ...
      const settle2 = await (await ethers.getContractFactory("BroadsideSettlement"))
        .deploy(await pauseReg.getAddress());
      ...
      const vault2 = await (await ethers.getContractFactory("BroadsidePaymentVault")).deploy();
