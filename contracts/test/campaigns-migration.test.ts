...
// BroadsideCampaignsMigrationLogic, reached via BroadsideCampaigns.migrateDelegate()
...
describe("BroadsideCampaigns — full migration via delegatecall logic", function () {
  ...
  beforeEach(async function () {
    ...
    pause = await (await ethers.getContractFactory("BroadsidePauseRegistry")).deploy(owner.address, adv.address, pubr.address);
    const pubs = await (await ethers.getContractFactory("BroadsidePublishers")).deploy(100, await pause.getAddress());
  ...
  it("migrateDelegate rejects any selector other than importCampaignFull", async function () {
    await v2.connect(gov).setMigrationLogic(await logic.getAddress());
    // the logic INHERITS BroadsideUpgradable.transferOwnership — a generic passthrough
