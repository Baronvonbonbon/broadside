...
describe("CampaignAllowlist + XcmBridge — upgrade migration", function () {
  ...
  it("BroadsideCampaignAllowlist copies the per-(campaign,publisher) allow set", async function () {
    const v1 = await (await ethers.getContractFactory("BroadsideCampaignAllowlist")).deploy();
  ...
  it("BroadsidePeopleChainXcmBridge migrates per-campaign escrow + config + sweeps DOT", async function () {
    const xcm = pubA.address, cache = pubB.address; // arbitrary non-zero (not called here)
    const v1 = await (await ethers.getContractFactory("BroadsidePeopleChainXcmBridge")).deploy(xcm, cache);
    await v1.setRouter(await router.getAddress());
