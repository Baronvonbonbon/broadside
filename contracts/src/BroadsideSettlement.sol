...
// EIP712 + ECDSA moved to BroadsideDualSig (alpha-4 EIP-170 carve-out).
// State, errors, non-Claim events, and the inline ICampaignsUserCapView
// interface live on BroadsideSettlementStorage (alpha-4 phase 8d-1) so the
...
// BroadsideUpgradable; IBroadsideSettlement stays on Settlement so the public
...
import "./interfaces/IBroadsidePublisherStake.sol";
...
import "./interfaces/IBroadsidePaymentVault.sol";
import "./interfaces/IBroadsideTokenRewardVault.sol";
import "./interfaces/IBroadsideCampaignLifecycle.sol";
import "./interfaces/IBroadsidePeopleChainIdentity.sol";
import "./interfaces/IBroadsidePowEngine.sol";
import "./interfaces/IBroadsidePublisherReputation.sol";
import "./interfaces/IBroadsideNullifierRegistry.sol";
import "./interfaces/IBroadsideSettlementRateLimiter.sol";
import "./interfaces/IBroadsideMintCoordinator.sol";

/// IBroadsideMintAuthority_Settle / IBroadsideEmissionEngine interface refs moved
/// to BroadsideMintCoordinator (alpha-4 EIP-170 carve-out). Settlement now
/// holds a single mintCoordinator pointer and calls `coordinate` once
/// per batch.

/// @title BroadsideSettlement
...
contract BroadsideSettlement is IBroadsideSettlement, BroadsideSettlementStorage {
    ...
    // ── DATUM token mint — carved out to BroadsideMintCoordinator ───────────
    ...
    // logic all moved to BroadsideMintCoordinator (alpha-4 EIP-170 carve-out).
    ...
    // ── BM-5: Rate limiter — carved out to BroadsideSettlementRateLimiter ──
    // ── FP-5: Nullifier registry — carved out to BroadsideNullifierRegistry ──
    ...
    // ── BM-8/BM-9: Publisher reputation — carved out to BroadsidePublisherReputation
    // Reputation state, the per-publisher acceptance counters, and the
    // anomaly views moved to BroadsidePublisherReputation so the module can be
    ...
    // -------------------------------------------------------------------------
    // #5: Per-impression PoW — carved out to BroadsidePowEngine
    // -------------------------------------------------------------------------
    // PoW state, difficulty curve, admin, and views moved to BroadsidePowEngine
    ...
    // sign time + a block-number deadline (parity with BroadsideRelay). Both
    ...
    // CLAIM_BATCH_TYPEHASH + EIP712 base moved to BroadsideDualSig
    ...
    constructor(address pauseRegistry_) {
        ...
        _pauseRegistry = IBroadsidePauseRegistry(pauseRegistry_);
    }
    ...
    function configure(
        ...
        _budgetLedger = IBroadsideBudgetLedger(budgetLedger_);
        _paymentVault = IBroadsidePaymentVault(paymentVault_);
        _lifecycle = IBroadsideCampaignLifecycle(lifecycle_);
    ...
    function setClaimValidator(address addr) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        _claimValidator = IBroadsideClaimValidator(addr);
    }
    ...
    function setRateLimiter(address addr) external onlyOwner {
        if (addr == address(0)) revert E00();
        if (_plumbingLocked) revert AlreadySet();
        _rateLimiter = IBroadsideSettlementRateLimiter(addr);
    ...
    function setPublishers(address addr) external onlyOwner {
        if (!(addr != address(0))) revert E00();
        if (_plumbingLocked) revert AlreadySet();
        _publishers = IBroadsidePublishers(addr);
    }
    ...
    function setTokenRewardVault(address addr) external onlyOwner {
        if (_plumbingLocked) revert AlreadySet();
        _tokenRewardVault = IBroadsideTokenRewardVault(addr);
    }
    ...
    function setCampaigns(address addr) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        if (!(addr != address(0))) revert E00();
        _campaigns = IBroadsideCampaigns(addr);
    }
    ...
    function setPublisherStake(address addr) external onlyOwner {
        if (!(addr != address(0))) revert E00();
        if (_plumbingLocked) revert AlreadySet();
        _publisherStake = IBroadsidePublisherStake(addr);
    }
    ...
    function setNullifierRegistry(address addr) external onlyOwner {
        if (addr == address(0)) revert E00();
        if (_plumbingLocked) revert AlreadySet();
        _nullifiers = IBroadsideNullifierRegistry(addr);
    ...
    function setReputationContract(address rep) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        _reputation = IBroadsidePublisherReputation(rep);
    ...
    function setIdentityRegistry(address addr) external onlyOwner {
        if (!(addr != address(0))) revert E00();
        if (_plumbingLocked) revert AlreadySet();
        _identityRegistry = IBroadsidePeopleChainIdentity(addr);
    ...
    // -------------------------------------------------------------------------
    // #5: PoW — moved to BroadsidePowEngine
    // -------------------------------------------------------------------------
    ...
    function setPowEngine(address engine) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        _powEngine = IBroadsidePowEngine(engine);
    ...
    function setClickRegistry(address addr) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        if (!(addr != address(0))) revert E00();
        _clickRegistry = IBroadsideClickRegistry(addr);
    }
    ...
    function setMintCoordinator(address coordinator) external onlyOwner {
        ...
        if (_plumbingLocked) revert AlreadySet();
        _mintCoordinator = IBroadsideMintCoordinator(coordinator);
        emit MintCoordinatorSet(coordinator);
    }

    // setMintAuthority / setEmissionEngine / setMintRate / setDustMintThreshold /
    // setDatumRewardSplit moved to BroadsideMintCoordinator.
    ...
    // -------------------------------------------------------------------------
    //
    // BroadsideSettlementLogicA and BroadsideSettlementLogicB inherit the same
    // BroadsideSettlementStorage abstract base as Settlement, so DELEGATECALL
    ...
    ///         (BroadsideSettlementStorage). Owner-settable; later upgrade-ladder
    ...
    ///         BroadsideClaimValidator.lockPlumbing / BroadsidePublishers locks.
    ...
    /// @inheritdoc IBroadsideSettlement
    /// @dev Thin dispatcher. The relay-side outer loop lives in
    ///      BroadsideSettlementLogicA, which in turn delegatecalls
    ///      BroadsideSettlementLogicB.processBatch for each batch. Both
    ///      Logic contracts share Settlement's storage via the
    ///      BroadsideSettlementStorage abstract base, so every SLOAD /
    ...
    ///      BroadsideSettlementLogicA via DELEGATECALL. Both relay entry
    ...
    /// @inheritdoc IBroadsideSettlement
    /// @dev Called by BroadsideDualSig after both EIP-712 signatures
    ...
    // on BroadsideSettlementStorage. Webapp, extension, SDK, relay-bot, and
    // tests address these by their original names; the storage rename is
    // internal-only.
    // ─────────────────────────────────────────────────────────────────────

    function budgetLedger() external view returns (IBroadsideBudgetLedger) { return _budgetLedger; }
    function paymentVault() external view returns (IBroadsidePaymentVault) { return _paymentVault; }
    function lifecycle() external view returns (IBroadsideCampaignLifecycle) { return _lifecycle; }
    function relayContract() external view returns (address) { return _relayContract; }
    function pauseRegistry() external view returns (IBroadsidePauseRegistry) { return _pauseRegistry; }
    function attestationVerifier() external view returns (address) { return _attestationVerifier; }
    function claimValidator() external view returns (IBroadsideClaimValidator) { return _claimValidator; }
    function publishers() external view returns (IBroadsidePublishers) { return _publishers; }
    function tokenRewardVault() external view returns (IBroadsideTokenRewardVault) { return _tokenRewardVault; }
    function campaigns() external view returns (IBroadsideCampaigns) { return _campaigns; }
    function publisherStake() external view returns (IBroadsidePublisherStake) { return _publisherStake; }
    function advertiserStake() external view returns (address) { return _advertiserStake; }
    function clickRegistry() external view returns (IBroadsideClickRegistry) { return _clickRegistry; }
    function mintCoordinator() external view returns (IBroadsideMintCoordinator) { return _mintCoordinator; }
    function rateLimiter() external view returns (IBroadsideSettlementRateLimiter) { return _rateLimiter; }
    function nullifiers() external view returns (IBroadsideNullifierRegistry) { return _nullifiers; }
    function reputation() external view returns (IBroadsidePublisherReputation) { return _reputation; }
    function identityRegistry() external view returns (IBroadsidePeopleChainIdentity) { return _identityRegistry; }
    function powEngine() external view returns (IBroadsidePowEngine) { return _powEngine; }
