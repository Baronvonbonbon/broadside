...
/// @dev Forward-declared shape of BroadsideSettlementLogicB.processBatch.
///      Used by the shared `_delegateProcessBatch` helper on
///      BroadsideSettlementStorage so both BroadsideSettlement (dual-sig path)
///      and BroadsideSettlementLogicA (relay outer loops) can dispatch into
...
interface IBroadsideSettlementLogicB_processBatch {
...
/// @title  BroadsideSettlementStorage
/// @notice Shared abstract storage base for BroadsideSettlement and the future
///         BroadsideSettlementLogicA / BroadsideSettlementLogicB contracts (phase
...
///         the same non-interface chain BroadsideSettlement had pre-phase-1
///         (ReentrancyGuard + BroadsideUpgradable) so ancestor slots stay at
///         their original offsets, then declares every Settlement-owned
///         state variable in its original order.
///
/// @dev    IBroadsideSettlement is NOT inherited here. Interfaces contribute no
...
///         standalone settlement contracts). BroadsideSettlement re-declares
///         `is IBroadsideSettlement` on its own line so it remains the public
///         settlement ABI surface.
///
/// @dev    NAMING: state variables use an underscore prefix (`_foo`)
///         so BroadsideSettlement can expose them as `function foo()` external
...
abstract contract BroadsideSettlementStorage is
    ReentrancyGuard,
    BroadsideUpgradable
{
    // ─────────────────────────────────────────────────────────────────────
    // Cross-contract references (one slot each, order preserved from the
    // pre-refactor BroadsideSettlement). Underscore prefix to avoid colliding
    // with the public getters BroadsideSettlement exposes.
    // ─────────────────────────────────────────────────────────────────────

    IBroadsideBudgetLedger      internal _budgetLedger;
    IBroadsidePaymentVault      internal _paymentVault;
    IBroadsideCampaignLifecycle internal _lifecycle;
    address                 internal _relayContract;
    /// @notice Demoted from `immutable` in phase 8d-1 so future
    ///         BroadsideSettlementLogic contracts can read this through shared
    ///         storage. Set once in BroadsideSettlement's constructor; no setter.
    IBroadsidePauseRegistry     internal _pauseRegistry;
    address                 internal _attestationVerifier;
    IBroadsideClaimValidator    internal _claimValidator;
    IBroadsidePublishers        internal _publishers;
    IBroadsideTokenRewardVault  internal _tokenRewardVault;
    IBroadsideCampaigns         internal _campaigns;
    IBroadsidePublisherStake    internal _publisherStake;
    address                 internal _advertiserStake;
    IBroadsideClickRegistry     internal _clickRegistry;

    // ── DATUM token mint -- carved out to BroadsideMintCoordinator (C8b) ────
    IBroadsideMintCoordinator internal _mintCoordinator;

    // ── BM-5 + FP-5 carve-outs (alpha-4 EIP-170) ────────────────────────
    IBroadsideSettlementRateLimiter internal _rateLimiter;
    IBroadsideNullifierRegistry     internal _nullifiers;

    // ── BM-8/BM-9 reputation -- carved out to BroadsidePublisherReputation ──
    IBroadsidePublisherReputation internal _reputation;
    ...
    IBroadsidePeopleChainIdentity internal _identityRegistry;
    ...
    // ── PoW -- carved out to BroadsidePowEngine ─────────────────────────────
    IBroadsidePowEngine internal _powEngine;
    ...
    // BroadsideSettlementLogicA and BroadsideSettlementLogicB carry the bytecode
    ...
    // Same pattern as BroadsideClaimValidator.plumbingLocked / BroadsidePublishers.
    bool internal _plumbingLocked;

    // ─────────────────────────────────────────────────────────────────────
    // Events
    //
    // ClaimSettled + ClaimRejected stay declared on IBroadsideSettlement (the
    ...
    // Shared types
    //
    // Memory-only struct; not state. Lives on the base so BroadsideSettlement
    // and BroadsideSettlementLogicB see the same definition. Aggregates one
    ...
    function _isPublisherRelay(IBroadsideSettlement.Claim[] calldata claims)
        internal
        view
        returns (bool)
    {
    ...
    ///                         BroadsideDualSig) can satisfy L2.
    ...
    /// @dev Forward one batch into BroadsideSettlementLogicB via DELEGATECALL.
    ///      Shared between BroadsideSettlement (dual-sig path) and
    ///      BroadsideSettlementLogicA (relay outer loops) so both invoke the
    ...
    function _delegateProcessBatch(
        ...
        (bool ok, bytes memory ret) = target.delegatecall(
            abi.encodeCall(
                IBroadsideSettlementLogicB_processBatch.processBatch,
