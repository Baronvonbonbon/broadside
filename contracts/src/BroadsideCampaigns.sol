...
import "./BroadsideCampaignsStorage.sol";
...
import "./interfaces/IBroadsideActivationBonds.sol";
// IBroadsideTagCurator / IBroadsideTagRegistry refs moved to BroadsideTagSystem
...
import "./BroadsideCampaignsMigrationLogic.sol";

/// @title BroadsideCampaigns (Core)
...
///         in BroadsideBudgetLedger per (campaignId, actionType).
contract BroadsideCampaigns is BroadsideCampaignsStorage {
    ...
    /// @dev Bounds match the per-publisher take rate range enforced by BroadsidePublishers
    ...
    // Tag caps + ceilings moved to BroadsideTagSystem (alpha-4 EIP-170 carve-out).
    ...
    // retuned without redeploying BroadsideCampaigns. Each parameter has:
    //
    ...
    // to BroadsideTagSystem (alpha-4 EIP-170 carve-out).
    ...
    // BroadsideTagSystem (alpha-4 EIP-170 carve-out). Campaigns retains a single
    ...
    ///         relay-signer pattern (BroadsidePublishers.relaySigner). When set,
    ...
    // ---- Allowlist snapshots (merged from BroadsideCampaignValidator) ----
    // Per-PUBLISHER's advertiser allowlist (set by the publisher on their inventory).

    // Multi-publisher allowlist state + setters moved to BroadsideCampaignAllowlist
    ...
    //              Read by ClaimValidator and passed as pub4 to BroadsideZKVerifier.verifyA.
    ...
    //              Passed as pub6 to BroadsideZKVerifier.verifyA — the user proves
    ...
    // identity level the user must hold (cached in BroadsidePeopleChainIdentity).
    ...
    // permitted any time. The check itself happens in BroadsideSettlement, which
    ...
    // BroadsideCampaignCreative (alpha-4 EIP-170 carve-out). BroadsideCampaigns
    // no longer tracks the per-campaign metadata hash, version, or
    // cooldown state -- the module owns 100% of creative resolution.

    // ---- Community reports moved to BroadsideReports (alpha-4 EIP-170 carve-out) ----

    // -------------------------------------------------------------------------
    // Bulletin Chain creative storage moved to BroadsideBulletinCreative
    ...
    // BroadsideCampaigns no longer references it. Frontends call the bulletin
    ...
    function acceptBudgetLedger() external {
        ...
        budgetLedger = IBroadsideBudgetLedger(c);
    ...
    // was prototyped here but does NOT fit under EIP-170 — BroadsideCampaigns is at
    ...
    // setMaxPublisherTags / setMaxCampaignTags moved to BroadsideTagSystem.
    // setMaxAllowedPublishers moved to BroadsideCampaignAllowlist.
    ...
    function setAllowlist(address addr) external onlyOwner whenPlumbingUnlocked {
        if (addr == address(0)) revert E00();
        allowlist = IBroadsideCampaignAllowlist(addr);
    ...
    function setTagSystem(address addr) external onlyOwner whenPlumbingUnlocked {
        if (addr == address(0)) revert E00();
        tagSystem = IBroadsideTagSystem(addr);
    ...
    // setTagCurator / lockTagCurator moved to BroadsideTagSystem
    // (alpha-4 EIP-170 carve-out).
    ...
    function setAdvertiserStake(address addr) external onlyOwner whenPlumbingUnlocked {
        if (!(addr != address(0))) revert E00();
        advertiserStake = IBroadsideAdvertiserStake(addr);
        emit AdvertiserStakeSet(addr);
    }

    // _isTagApproved / listApprovedTags moved to BroadsideTagSystem.
    ...
    /// @inheritdoc IBroadsideCampaigns
    ...
    function _createCampaign(
        ...
        // requiredTags length check moved to BroadsideTagSystem.initializeCampaignTags.
        ...
        bool allowlistWasEnabled;
        {
            ...
            if (p.publisher != address(0)) {
                ...
                IBroadsidePublishers.Publisher memory pub = publishers.getPublisher(p.publisher);
        ...
        // Delegate tag validation + storage to BroadsideTagSystem.
        ...
        if (p.activationBondAmount > 0 && activationBonds != address(0)) {
            IBroadsideActivationBonds(activationBonds).openBond{value: p.activationBondAmount}(
    ...
    // BroadsideCampaignCreative (alpha-4 EIP-170 carve-out).
    ...
    // Multi-publisher allowlist moved to BroadsideCampaignAllowlist.
    ...
    ///         in BroadsidePeopleChainIdentity to settle on this campaign.
    ///
    ...
    /// @notice Delegating view: reads required-tag set from BroadsideTagSystem.
    ...
    /// @notice Delegating view: reads publisher-tags snapshot from BroadsideTagSystem.
    ...
    // full-import loop lives in BroadsideCampaignsMigrationLogic, reached via
    ...
    ///         contract — it lives only in BroadsideCampaignsMigrationLogic.
    /// @dev    The leading selector is constrained to `importCampaignFull` so a
    ///         governance tx can't DELEGATECALL the logic's INHERITED
    ///         BroadsideUpgradable surface (setRouter / transferOwnership / migrate),
    ...
    function migrateDelegate(bytes calldata data) external onlyGovernance {
        ...
        if (bytes4(data[:4]) != BroadsideCampaignsMigrationLogic.importCampaignFull.selector) revert E00();
