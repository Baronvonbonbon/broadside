...
/// @title  BroadsideSettlementLogicB
...
///
/// @dev    Invoked exclusively via DELEGATECALL from BroadsideSettlement (and,
///         in phase 8d-4, from BroadsideSettlementLogicA's relay loops).
///         The shared `BroadsideSettlementStorage` base guarantees identical
...
///         lives on BroadsideSettlementStorage and is shared).
///
/// @dev    LAYOUT INVARIANT: never declare additional state in this
///         contract. New state goes on BroadsideSettlementStorage.
contract BroadsideSettlementLogicB is BroadsideSettlementStorage {
    ...
    function processBatch(
        ...
        if (_userPaused[user]) {
            for (uint256 j = 0; j < claims.length; j++) {
                ...
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 27);
            }
        ...
        if (address(_campaigns) != address(0)) {
            ...
            if (advOk && adv != address(0) && _userBlocksAdvertiser[user][adv]) {
                for (uint256 j = 0; j < claims.length; j++) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 28);
                }
        ...
        if (_userMinAssurance[user] >= 3 && address(_campaigns) != address(0)) {
            ...
            if (!zkOk || !reqZk) {
                for (uint256 j = 0; j < claims.length; j++) {
                    ...
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 26);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }
        ...
        // at >= that level in BroadsidePeopleChainIdentity, regardless of which
        ...
        // can't silently downgrade an identity-gated campaign to no gate.
        {
            ...
            if (effMinId > 0) {
                ...
                if (!ok) {
                    for (uint256 j = 0; j < claims.length; j++) {
                        result.rejectedCount++;
                        emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 30);
                    }
                    return (result.settledCount, result.rejectedCount, result.totalPaid);
                }
            }
        ...
        // `_assuranceDecision` helper on BroadsideSettlementStorage (phase 8d
        ...
        if (address(_campaigns) != address(0)) {
            ...
            if (!accept) {
                for (uint256 j = 0; j < claims.length; j++) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, reasonCode);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }
        ...
        if (interval > 0) {
            ...
            if (lastBlock != 0 && block.number < lastBlock + interval) {
                for (uint256 j = 0; j < claims.length; j++) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 18);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }

        // Safe rollout: reputation gate (delegated to BroadsidePublisherReputation)
        if (address(_reputation) != address(0) && claims.length > 0) {
            if (!_reputation.canSettle(claims[0].publisher)) {
                for (uint256 j = 0; j < claims.length; j++) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 20);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }
        ...
        if (batchPublisher != address(0) && address(_publishers) != address(0)) {
            ...
            if (blocked) {
                for (uint256 j = 0; j < claims.length; j++) {
                    ...
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, 11);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }
        ...
        if (claims.length > 0) {
            ...
            if (!ctx.ok) {
                for (uint256 j = 0; j < claims.length; j++) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, startNonce + j, ctx.reasonCode);
                }
                return (result.settledCount, result.rejectedCount, result.totalPaid);
            }
        }
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            IBroadsideSettlement.Claim calldata claim = claims[i];
            ...
            if (claim.actionType != batchActionType) {
                result.rejectedCount++;
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 21);
                gapFound = true;
                continue;
            }
            ...
            if (gapFound) {
                result.rejectedCount++;
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 1);
                continue;
            }
            ...
            if (!ok) {
                ...
                gapFound = true;
                result.rejectedCount++;
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, reasonCode);
                continue;
            }
            ...
            if (newTotal > MAX_USER_EVENTS) {
                result.rejectedCount++;
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 13);
                gapFound = true;
                continue;
            }
            ...
            if (capMaxHoisted > 0 && capWinHoisted > 0) {
                ...
                if (cur + claim.eventCount > uint256(capMaxHoisted)) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 29);
                    gapFound = true;
                    continue;
                }
            ...
            //       Delegated to BroadsideSettlementRateLimiter (atomic try-consume).
            ...
            if (address(_rateLimiter) != address(0) && claim.actionType == 0) {
                if (!_rateLimiter.tryConsume(claim.publisher, claim.eventCount)) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 14);
                    gapFound = true;
                    continue;
                }
            }
            ...
            if (!publisherStakedHoisted) {
                result.rejectedCount++;
                emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 15);
                gapFound = true;
                continue;
            }
            ...
            //       Delegated to BroadsideNullifierRegistry.tryConsume which marks
            ...
            if (
                ...
                if (!_nullifiers.tryConsume(campaignId, claimNullifier)) {
                    result.rejectedCount++;
                    emit IBroadsideSettlement.ClaimRejected(campaignId, user, assignedNonce, 19);
                    gapFound = true;
                    continue;
                }
            }
            ...
            emit IBroadsideSettlement.ClaimSettled(
                campaignId,
                user,
                claim.publisher,
                claim.eventCount,
        ...
        // ── DATUM token mint — delegated to BroadsideMintCoordinator ──
