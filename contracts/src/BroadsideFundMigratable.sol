...
///         doesn't apply.
abstract contract BroadsideFundMigratable is BroadsidePlumbingLockable {
    ...
    function migrateFundsTo(address successor) external virtual onlyGovernance {
        ...
        if (bal > 0) BroadsideFundMigratable(payable(successor)).acceptMigration{value: bal}();
    }
