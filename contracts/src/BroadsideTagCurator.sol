...
/// @title BroadsideTagCurator
/// @notice M5-fix: council-driven tag-approval registry. Plugs into
///         BroadsideCampaigns as `setTagCurator(this)`. Mutations are gated to a
///         designated council address (typically the BroadsideCouncil contract),
///         which drives approve/remove via its propose+vote+execute pipeline.
///
///         Mirrors the cypherpunk lock pattern of BroadsideCouncilBlocklistCurator:
///         the owner (Timelock) can rotate the council pointer up until
///         `lockCouncil()` is called, after which the pointer is permanently
///         frozen.
///
///         BroadsideCampaigns OR-merges the curator's verdict with its own
///         `approvedTags` mapping, so a deployer can use either path or
///         both. When no curator is wired, `approvedTags` is consulted alone.
contract BroadsideTagCurator is IBroadsideTagCurator, PaseoSafeSender, BroadsideUpgradable {
    ...
    /// @inheritdoc IBroadsideTagCurator
    ...
    // -------------------------------------------------------------------------
    //
    // Mirrors the bonded-appeal pattern from BroadsideCouncilBlocklistCurator.
