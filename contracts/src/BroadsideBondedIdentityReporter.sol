...
/// @dev Minimal write-side view of BroadsidePeopleChainIdentity. The bonded
...
/// @title  BroadsideBondedIdentityReporter
/// @notice Permissionless bonded multi-reporter set for People Chain
///         identity attestations. Mirrors `BroadsideStakeRootV2`:
...
///
/// @dev    The cache (`BroadsidePeopleChainIdentity`) must authorize this
...
contract BroadsideBondedIdentityReporter is BroadsideUpgradable, PaseoSafeSender {
    ...
    // live in the migrated BroadsidePeopleChainIdentity cache, not here.
    ...
    ///      separately-migrated BroadsidePeopleChainIdentity cache. `cache` is
    ///      re-wired. Native DOT (stakes + payouts + live bonds) moves via
    ///      `migrateFundsTo`.
    function _migrate(address oldContract) internal override {
        BroadsideBondedIdentityReporter old = BroadsideBondedIdentityReporter(payable(oldContract));
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideBondedIdentityReporter(payable(successor)).acceptMigration{value: bal}();
    }
