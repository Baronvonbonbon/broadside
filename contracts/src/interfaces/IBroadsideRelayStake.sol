...
///             AND no exit pending. BroadsideRelay consults this view to gate the
///             authorized-relayer set (pattern (b) augment: stake OR manual
///             authorization).
///           - `slash(relay, amount, recipient, reasonCode)` is callable only
///             by the wired governance contract (BroadsideRelayGovernance).
...
interface IBroadsideRelayStake {
    ...
    // PlumbingLocked is provided by BroadsidePlumbingLockable (the contract emits it).
