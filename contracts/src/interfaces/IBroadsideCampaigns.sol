...
///         BroadsideBudgetLedger on a per-(campaignId, actionType) basis.
interface IBroadsideCampaigns {
    // -------------------------------------------------------------------------
    // Enums
    // -------------------------------------------------------------------------
    ...
    /// @notice Core campaign state (budget fields live in BroadsideBudgetLedger,
    ///         creative storage in BroadsideCampaignCreative).
    struct Campaign {
        address advertiser;
        address publisher;
    ...
    // Multi-publisher allowlist views moved to BroadsideCampaignAllowlist
    // (alpha-4 EIP-170 carve-out).

    // Targeting views moved to BroadsideTagSystem (alpha-4 EIP-170 carve-out).

    // Report views moved to BroadsideReports (alpha-4 EIP-170 carve-out).
}
