...
/// @title  BroadsidePublisherReputation
/// @notice Publisher acceptance-rate tracker + anomaly detector. Carved out
///         of BroadsideSettlement so the module is independently upgradable
...
///         settlement paths flow through BroadsideSettlement._processBatch,
...
contract BroadsidePublisherReputation is IBroadsidePublisherReputation, BroadsideUpgradable {
    ...
    /// @inheritdoc IBroadsidePublisherReputation
    ...
    function _settledOf(address p) internal view returns (uint256 v) {
        ...
        if (pred != address(0)) v += BroadsidePublisherReputation(pred).repTotalSettled(p);
    }
    function _rejectedOf(address p) internal view returns (uint256 v) {
        v = _repTotalRejected[p];
        address pred = migrationSource;
        if (pred != address(0)) v += BroadsidePublisherReputation(pred).repTotalRejected(p);
    }
    function _campSettledOf(address p, uint256 c) internal view returns (uint256 v) {
        v = _repCampaignSettled[p][c];
        address pred = migrationSource;
        if (pred != address(0)) v += BroadsidePublisherReputation(pred).repCampaignSettled(p, c);
    }
    function _campRejectedOf(address p, uint256 c) internal view returns (uint256 v) {
        v = _repCampaignRejected[p][c];
        address pred = migrationSource;
        if (pred != address(0)) v += BroadsidePublisherReputation(pred).repCampaignRejected(p, c);
    }
    ...
    function _migrate(address oldContract) internal override {
        minReputationScore = BroadsidePublisherReputation(oldContract).minReputationScore();
    }
}
