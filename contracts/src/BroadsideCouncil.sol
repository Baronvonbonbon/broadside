...
contract BroadsideCouncil is BroadsideUpgradable, ReentrancyGuard {
    ...
    ///         Mirrors BroadsidePublishers.relaySigner + BroadsideCampaigns.advertiserRelaySigner.
    ...
    function _migrate(address oldContract) internal override {
        BroadsideCouncil old = BroadsideCouncil(payable(oldContract));
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideCouncil(payable(successor)).acceptMigration{value: bal}();
    }
