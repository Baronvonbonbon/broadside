...
contract BroadsideMintAuthority is BroadsideUpgradable {
    ...
    IBroadsidePauseRegistry_Mint public pauseRegistry;
    ...
    function setPauseRegistry(address _pauseRegistry) external onlyOwner {
        ...
        pauseRegistry = IBroadsidePauseRegistry_Mint(_pauseRegistry);
    ...
    /// @notice Called by BroadsideSettlement on every settled claim.
    ...
    function mintForSettlement(
        ...
        if (userAmt > 0)       IBroadsideWrapper(wrapper).mintTo(user,       userAmt);
        if (publisherAmt > 0)  IBroadsideWrapper(wrapper).mintTo(publisher,  publisherAmt);
        if (advertiserAmt > 0) IBroadsideWrapper(wrapper).mintTo(advertiser, advertiserAmt);

        emit MintedForSettlement(user, publisher, advertiser, total);
    }

    /// @notice Called by BroadsideVesting on each release().
    ...
    function mintForVesting(address recipient, uint256 amount) external whenNotFrozen {
        ...
        IBroadsideWrapper(wrapper).mintTo(recipient, amount);
