...
///
///         Fee inflow path:
///           1. BroadsidePaymentVault accrues `pendingFeeShare` on each settlement.
///           2. Anyone (off-chain bot, dev tooling, the user themselves) calls
///              `sweep()` which pulls accrued DOT from PaymentVault into this
///              contract via the IBroadsidePaymentVault_FeeShare interface.
...
contract BroadsideFeeShare is BroadsideUpgradable, PaseoSafeSender {
    ...
    /// @notice BroadsidePaymentVault address — source for protocol-fee sweeps.
    ...
    /// @notice Wire the BroadsidePaymentVault address so `sweep()` can pull fees.
    ...
    ///         pulling from `BroadsidePaymentVault.pendingFeeShare`.
