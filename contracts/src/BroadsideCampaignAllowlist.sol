...
/// @title  BroadsideCampaignAllowlist
/// @notice Per-campaign publisher allowlist + take-rate snapshot. Carved
///         out of BroadsideCampaigns for mainnet EIP-170.
///
/// @dev    Two writer paths:
///           1. `initializeFor(campaignId, publisher, takeRate)` -- gated
///              to `onlyCampaigns`. Called from BroadsideCampaigns._createCampaign
...
contract BroadsideCampaignAllowlist is
    IBroadsideCampaignAllowlist,
    BroadsideUpgradable,
    ReentrancyGuard
{
    ...
    IBroadsideChallengeBonds public challengeBonds; // optional; address(0) disables bond locking
    IBroadsideTagSystem    public tagSystem;        // optional; address(0) skips tag-match check
    ...
    function setChallengeBonds(address addr) external onlyOwner {
        if (plumbingLocked) revert LockedAlready();
        challengeBonds = IBroadsideChallengeBonds(addr); // address(0) is valid (feature off)
        emit ChallengeBondsSet(addr);
    }

    function setTagSystem(address addr) external onlyOwner {
        if (plumbingLocked) revert LockedAlready();
        tagSystem = IBroadsideTagSystem(addr); // address(0) skips tag-match check
    ...
    function addAllowedPublisher(uint256 campaignId, address publisher) external payable nonReentrant whenNotFrozen {
        ...
        if (!(st == IBroadsideCampaigns.CampaignStatus.Pending || st == IBroadsideCampaigns.CampaignStatus.Active)) revert E22();
        if (publisher == address(0)) revert E00();
        if (allowed[campaignId][publisher]) revert E71();
    ...
    function _validateAndSeat(uint256 campaignId, address publisher, address advertiser) internal {
        ...
        IBroadsidePublishers.Publisher memory pub = publishers.getPublisher(publisher);
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Views (IBroadsideCampaignAllowlist)
    // ─────────────────────────────────────────────────────────────────────
    ...
    /// @notice 0 = OPEN, 1 = ALLOWLIST. Mirrors the historic BroadsideCampaigns surface.
    ...
    function _migrate(address oldContract) internal override {
        BroadsideCampaignAllowlist old = BroadsideCampaignAllowlist(oldContract);
