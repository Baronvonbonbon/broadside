...
/// @title BroadsideAttestationVerifier
...
contract BroadsideAttestationVerifier is EIP712, BroadsideUpgradable {
    ...
    ///         `BroadsideRouter.upgradeContract` path instead of
    ...
    IBroadsideSettlement public immutable settlement;
    IBroadsideCampaignsSettlement public immutable campaigns;
    ...
    constructor(address _settlement, address _campaigns, address _pauseRegistry)
        EIP712("BroadsideAttestationVerifier", "1")
    {
    ...
    struct AttestedBatch {
        ...
        IBroadsideSettlement.Claim[] claims;
        ...
        returns (IBroadsideSettlement.SettlementResult memory result)
    {
        ...
        IBroadsideSettlement.ClaimBatch[] memory forwardBatches =
            new IBroadsideSettlement.ClaimBatch[](batches.length);
        ...
        for (uint256 b = 0; b < batches.length; b++) {
            ...
            if (expectedPublisher == address(0)) {
                ...
                // provided by BroadsideClaimValidator downstream, which checks publisher
                // registration in BroadsidePublishers before accepting settlement.
            ...
            // One-chain-per-call guard (E87). Same rationale as BroadsideRelay: this
            ...
            IBroadsideSettlement.Claim[] memory memoryClaims =
                new IBroadsideSettlement.Claim[](ab.claims.length);
            for (uint256 i = 0; i < ab.claims.length; i++) {
