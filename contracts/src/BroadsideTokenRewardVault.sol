...
/// @title BroadsideTokenRewardVault
...
///
///         Separated from BroadsidePaymentVault to keep ETH-native accounting isolated.
///         Only supports EVM-native ERC-20 tokens (not native Asset Hub assets).
contract BroadsideTokenRewardVault is IBroadsideTokenRewardVault, ReentrancyGuard, BroadsidePlumbingLockable {
    ...
    // -------------------------------------------------------------------------
    //
    // Mirrors the pattern from BroadsidePaymentVault.setRecoveryAddress: users
    ...
    /// @notice BroadsideCouncil — emergency toggle authority (instant N-of-M).
    ...
    /// @inheritdoc IBroadsideTokenRewardVault
    ...
    function _migrate(address oldContract) internal override {
        BroadsideTokenRewardVault old = BroadsideTokenRewardVault(payable(oldContract));
