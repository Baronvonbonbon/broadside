...
/// @dev Minimal write-side view of BroadsidePeopleChainIdentity for the bridge.
///      Kept inline so the read-only IBroadsidePeopleChainIdentity interface
...
/// @title BroadsidePeopleChainXcmBridge
...
///
///         The bridge becomes `xcmDispatcher` on BroadsidePeopleChainIdentity.
...
///         anti-Sybil.
contract BroadsidePeopleChainXcmBridge is BroadsideFundMigratable, PaseoSafeSender {
    ...
    /// @notice BroadsideCampaigns reference for advertiser auth on
    ...
    /// @notice Per-campaign refresh budget (mirrors BroadsideCampaigns'
    ...
    function withdrawXcmRefreshEscrow(
        ...
        address adv = IBroadsideCampaignsAdvertiser(campaignsContract)
    ...
    ///      backing the escrow moves via `migrateFundsTo` (BroadsideFundMigratable).
    function _migrate(address oldContract) internal override {
        BroadsidePeopleChainXcmBridge old = BroadsidePeopleChainXcmBridge(payable(oldContract));
