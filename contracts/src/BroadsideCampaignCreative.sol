...
///         reference for every campaign. Carved out of BroadsideCampaigns
...
///         state and BroadsideCampaigns no longer references it.
///
...
///
///         Authorization comes from a read against BroadsideCampaigns:
...
contract BroadsideCampaignCreative is
    IBroadsideCampaignCreative,
    BroadsideUpgradable,
    PaseoSafeSender
{
    ...
    IBroadsidePauseRegistry public pauseRegistry;
    bool public plumbingLocked;

    // ─────────────────────────────────────────────────────────────────────
    // Constants (mirror values that previously lived in BroadsideCampaigns)
    // ─────────────────────────────────────────────────────────────────────
    ...
    ///         METADATA_COOLDOWN_BLOCKS on BroadsideCampaigns.setMetadata so
    ...
    function setPauseRegistry(address addr) external onlyOwner {
        if (plumbingLocked) revert LockedAlready();
        if (addr == address(0)) revert E00();
        pauseRegistry = IBroadsidePauseRegistry(addr);
    ...
    function setMetadata(uint256 campaignId, bytes32 metadataHash) external whenNotFrozen {
        ...
        if (campaigns.getCampaignStatus(campaignId) != IBroadsideCampaigns.CampaignStatus.Pending) {
    ...
    function setBulletinCreative(
        ...
        if (br.cidDigest != bytes32(0)) {
            IBroadsideCampaigns.CampaignStatus st = campaigns.getCampaignStatus(campaignId);
            if (st != IBroadsideCampaigns.CampaignStatus.Pending) {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Views (IBroadsideBulletinCreative)
    // ─────────────────────────────────────────────────────────────────────
    ...
    /// @notice Public accessor matching the previous BroadsideCampaigns layout
    ...
    /// @inheritdoc IBroadsideCampaignCreative
    ...
    function _migrate(address oldContract) internal override {
        BroadsideCampaignCreative old = BroadsideCampaignCreative(payable(oldContract));
    ...
    ///         (Hand-rolled rather than via BroadsideFundMigratable because this
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideCampaignCreative(payable(successor)).acceptMigration{value: bal}();
    }
