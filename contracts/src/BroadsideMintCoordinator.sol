...
/// @dev Inline interface to BroadsideMintAuthority's settlement entry point.
...
/// @title  BroadsideMintCoordinator
/// @notice Carve-out of DATUM-token emission orchestration. Previously the
///         ~36-line inline mint block inside BroadsideSettlement._processBatch,
...
contract BroadsideMintCoordinator is IBroadsideMintCoordinator, BroadsideUpgradable, ParameterRetuneGuard {
    ...
    function _migrate(address oldContract) internal override {
        BroadsideMintCoordinator o = BroadsideMintCoordinator(oldContract);
    ...
    /// @notice BroadsideCouncil — emergency authority that can flip the emission
    ...
    event BroadsideRewardSplitSet(uint16 userBps, uint16 publisherBps, uint16 advertiserBps);
    event BroadsideMintFailed(address indexed user, address indexed publisher, address indexed advertiser, uint256 totalMint);
    ...
    function setDatumRewardSplit(uint16 userBps, uint16 publisherBps, uint16 advertiserBps) external onlyOwnerOrPG whenNotFrozen {
        ...
        emit BroadsideRewardSplitSet(userBps, publisherBps, advertiserBps);
    }
    ...
    /// @inheritdoc IBroadsideMintCoordinator
    function coordinate(
        address user,
        address publisher,
        address advertiser,
        ...
        try IMintAuthority(mintAuthority).mintForSettlement(
            ...
            emit BroadsideMintFailed(user, publisher, advertiser, totalMint);
        }
    }
}
