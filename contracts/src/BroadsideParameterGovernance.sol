...
import "./interfaces/IBroadsideParameterGovernance.sol";
import "./interfaces/IBroadsidePauseRegistry.sol";
import "./BroadsideUpgradable.sol";
import "./PaseoSafeSender.sol";

/**
 * BroadsideParameterGovernance — T1-B
 *
 ...
 * it governs (BroadsidePublisherStake, BroadsidePublisherGovernance, BroadsideNullifierRegistry).
 *
 * Governance flow:
 ...
 *
 * Conviction multipliers (×1 to ×21) and lockups match BroadsidePublisherGovernance.
 * Error codes: E00 zero addr, E02 transfer fail, E03 zero value,
 *              E11 bad value, E18 not owner, E40 proposal state/condition error,
 *              E57 reentrancy.
 */
contract BroadsideParameterGovernance is IBroadsideParameterGovernance, BroadsideUpgradable, PaseoSafeSender {
    ...
    function _migrate(address oldContract) internal override {
        BroadsideParameterGovernance old = BroadsideParameterGovernance(payable(oldContract));
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideParameterGovernance(payable(successor)).acceptMigration{value: bal}();
    }
