...
/// @title MockCampaigns
/// @notice Test-only mock for BroadsideCampaigns. Supports direct status/data manipulation
...
contract MockCampaigns {
    ...
    // -------------------------------------------------------------------------
    // IBroadsideCampaigns interface (subset used by other contracts)
    // -------------------------------------------------------------------------

    /// @dev 3-value return matching alpha-3 IBroadsideCampaignsMinimal interface.
    ...
    function callOpenBond(uint256 campaignId, address creator) external payable {
        ...
        IBroadsideActivationBonds(activationBonds).openBond{value: msg.value}(campaignId, creator);
    }
    ...
    // -------------------------------------------------------------------------
    // IBroadsidePublishers stubs (used as publishers placeholder in settlement tests)
    // -------------------------------------------------------------------------
    ...
    /// @dev Per-publisher relay signer (mirrors BroadsidePublishers.relaySigner mapping).
    ...
    /// @dev M6: per-advertiser relay signer (mirrors BroadsideCampaigns.advertiserRelaySigner mapping).
    ...
    // -------------------------------------------------------------------------
    // IBroadsideCampaignsSettlement stubs (used for ZK proof check in ClaimValidator)
    // -------------------------------------------------------------------------
    ...
    //
    // Mirrors the safe variants on BroadsideCampaigns so tests using
    ...
    // `campaigns[id].advertiser != address(0)` (same as BroadsideCampaigns).
    ...
    function getCampaignRewardTokenSafe(uint256 /*campaignId*/) external pure returns (bool ok, address token) {
        ...
        // reward should use the real BroadsideCampaigns; for the rest, returning
