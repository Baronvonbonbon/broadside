...
/// @title BroadsideTagRegistry
/// @notice Stake-gated tag namespace with Schelling-point arbitration. Pairs
///         with the curated `BroadsideTagCurator` and the per-actor `Any` lane as
...
contract BroadsideTagRegistry is IBroadsideTagRegistry, BroadsideUpgradable, ReentrancyGuard {
    ...
    /// @inheritdoc IBroadsideTagRegistry
    ...
    function _migrate(address oldContract) internal override {
        BroadsideTagRegistry old = BroadsideTagRegistry(oldContract);
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        require(BroadsideTagRegistry(successor).datum() == datum, "token-mismatch");
