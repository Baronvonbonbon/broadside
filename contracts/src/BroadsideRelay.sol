...
import "./interfaces/IBroadsideCampaignsSettlement.sol";
import "./interfaces/IBroadsidePauseRegistry.sol";
import "./interfaces/IBroadsideRelayStake.sol";

/// @title BroadsideRelay
...
contract BroadsideRelay is BroadsideUpgradable, EIP712 {
    ...
    // dual-sig path's CLAIM_BATCH_TYPEHASH in BroadsideSettlement.
    ...
    // themselves). Matches BroadsideAttestationVerifier's anchored typehash.
    ...
    IBroadsideSettlement public settlement;
    IBroadsideCampaignsSettlement public campaigns;
    ...
    ///         pre-existing flow.
    IBroadsideRelayStake public relayStake;
    ...
    constructor(address _settlement, address _campaigns, address _pauseRegistry)
        EIP712("BroadsideRelay", "1")
    {
        require(_settlement != address(0), "E00");
        require(_campaigns != address(0), "E00");
        require(_pauseRegistry != address(0), "E00");
        settlement = IBroadsideSettlement(_settlement);
        campaigns = IBroadsideCampaignsSettlement(_campaigns);
    ...
    function setSettlement(address addr) external onlyOwner {
        ...
        settlement = IBroadsideSettlement(addr);
    ...
    function setCampaigns(address addr) external onlyOwner {
        require(!plumbingLocked, "locked");
        require(addr != address(0), "E00");
        campaigns = IBroadsideCampaignsSettlement(addr);
    ...
    /// @notice G-1 close: wire BroadsideRelayStake. Pattern (b) augment — a
    ...
    function setRelayStake(address addr) external onlyOwner {
        require(!plumbingLocked, "locked");
        relayStake = IBroadsideRelayStake(addr);
    ...
    function settleClaimsFor(IBroadsideSettlement.SignedClaimBatch[] calldata batches)
        external
        whenNotFrozen
        returns (IBroadsideSettlement.SettlementResult memory result)
    {
        ...
        IBroadsideSettlement.ClaimBatch[] memory forwardBatches = new IBroadsideSettlement.ClaimBatch[](batches.length);
        ...
        for (uint256 b = 0; b < batches.length; b++) {
            IBroadsideSettlement.SignedClaimBatch calldata sb = batches[b];
            ...
            if (sb.publisherSig.length > 0) {
                ...
                if (expectedPub != address(0)) {
                    ...
                    // Matches BroadsideDualSig._hashClaims for symmetry.
            ...
            IBroadsideSettlement.Claim[] memory memoryClaims = new IBroadsideSettlement.Claim[](sb.claims.length);
            for (uint256 i = 0; i < sb.claims.length; i++) {
                memoryClaims[i] = sb.claims[i];
            }
            forwardBatches[validCount] = IBroadsideSettlement.ClaimBatch({
                user: sb.user,
        ...
        if (validCount > 0) {
            IBroadsideSettlement.ClaimBatch[] memory toForward = forwardBatches;
            if (validCount < batches.length) {
                toForward = new IBroadsideSettlement.ClaimBatch[](validCount);
