...
import "./interfaces/IBroadsideDualSig.sol";
...
/// @title  BroadsideDualSig
/// @notice Carve-out of BroadsideSettlement's `settleSignedClaims` path. Verifies
...
///
/// @dev    Domain `"BroadsideSettlement" v"1"` preserved so off-chain signers
    ...
    EIP712
{
    ...
    /// @dev EIP-712 domain pinned to ("BroadsideSettlement", "1") so existing
    ///      off-chain signers keep producing valid digests.
    constructor() EIP712("BroadsideSettlement", "1") {}
        ...
        returns (IBroadsideSettlement.SettlementResult memory result)
    {
        ...
        for (uint256 b = 0; b < batches.length; b++) {
            IBroadsideSettlement.SignedClaimBatch calldata batch = batches[b];
            ...
            // so the dual-sig path's authorization model matches BroadsideRelay.
            ...
            IBroadsideSettlement.SettlementResult memory sub =
    ...
    function _hashClaims(IBroadsideSettlement.Claim[] calldata claims) internal pure returns (bytes32) {
    ...
    function _batchPublisher(IBroadsideSettlement.Claim[] calldata claims) internal pure returns (address) {
