...
///           5. BroadsideClaimValidator calls hasUnclaimed to confirm the session exists.
///           6. BroadsideSettlement calls markClaimed after the claim settles — prevents replay.
///
///         Security properties:
...
contract BroadsideClickRegistry is IBroadsideClickRegistry, BroadsideUpgradable {
    ...
    /// @inheritdoc IBroadsideClickRegistry
    ...
    function sessionStatus(bytes32 sh) public view returns (uint8) {
        ...
        try BroadsideClickRegistry(pred).sessionStatus(sh) returns (uint8 s) { return s; } catch { return 0; }
    }
    ...
    function _effectiveStatus(
        ...
        address pred = migrationSource;
        if (pred == address(0)) return 0;
        try BroadsideClickRegistry(pred).sessionStatus(sh) returns (uint8 s) {
            return s;
        } catch {
            if (IBroadsideClickRegistry(pred).hasUnclaimed(user, campaignId, impressionNonce)) return 1;
            return 0;
        }
    }
}
