...
contract BroadsideBudgetLedger is IBroadsideBudgetLedger, PaseoSafeSender, BroadsideFundMigratable {
    ...
    // v1 escrow into this build (BroadsideUpgradable.migrate requires old < new).
    ...
    ///         Mirrors BroadsideSettlement / BroadsideClaimValidator.
    ...
    // M-1: AdvertiserRefundQueued / AdvertiserRefundClaimed declared on IBroadsideBudgetLedger.
    ...
    // now provided by BroadsidePlumbingLockable.

    // -------------------------------------------------------------------------
    // Budget initialization (Campaigns only — once per pot)
    // -------------------------------------------------------------------------

    /// @inheritdoc IBroadsideBudgetLedger
    ...
    ///      BroadsideUpgradable.migrate (governance-gated; old frozen, higher version).
    ...
    /// @notice U3: gas-paginated override of `BroadsideUpgradable.migrate()`. Copies
    ...
    function migrate(address oldContract) external override onlyGovernanceOrRouter {
        ...
        BroadsideBudgetLedger old = BroadsideBudgetLedger(payable(oldContract));
    ...
    // migrateFundsTo + acceptMigration (native sweep) provided by BroadsideFundMigratable.
