...
/// @title  BroadsideRelayStake
/// @notice G-1 first close: bond gate + slash hook for the relay role.
///         Mirrors BroadsidePublisherStake / BroadsideAdvertiserStake in shape.
...
///           1. BroadsideRelay reads `isAuthorized(addr)` to gate its
///              authorized-relayer set (pattern (b) augment: pass if
///              EITHER manually authorized OR adequately staked).
///           2. BroadsideRelayGovernance calls `slash(...)` when a fraud
...
///         Both phase-gated on OpenGov via BroadsideUpgradable.whenOpenGovPhase.
contract BroadsideRelayStake is
    IBroadsideRelayStake,
    PaseoSafeSender,
    BroadsideFundMigratable
{
    function version() public pure virtual override returns (uint256) { return 1; }

    // ── Wiring (lock-once via plumbingLocked) ───────────────────────────
    address public relayContractAddr;           // BroadsideRelay
    address public governance;                  // BroadsideRelayGovernance
    ...
    ///         NOT exit-requested. Consumed by BroadsideRelay's gate (pattern (b)
    ...
    function _migrate(address oldContract) internal override {
        BroadsideRelayStake old = BroadsideRelayStake(payable(oldContract));
