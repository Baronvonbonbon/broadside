...
import "./BroadsideFundMigratable.sol";
import "./PaseoSafeSender.sol";

interface IBroadsideCampaignsForMute {
    function getCampaignAdvertiser(uint256 campaignId) external view returns (address);
}

/// @title BroadsideActivationBonds
/// @notice Optimistic-activation collateral. See IBroadsideActivationBonds.
///
...
contract BroadsideActivationBonds is IBroadsideActivationBonds, PaseoSafeSender, BroadsideFundMigratable {
    ...
    ///      settlement (matches BroadsideChallengeBonds.pendingBondReturn).
    ...
    // fundsMigratedOut + migrateFundsTo + acceptMigration provided by BroadsideFundMigratable.
    ...
    /// @inheritdoc IBroadsideActivationBonds
    ...
    function activate(uint256 campaignId) external nonReentrant whenNotFrozen {
        ...
        (uint8 status,,) = IBroadsideCampaignsMinimal(campaignsContract).getCampaignForSettlement(campaignId);
        ...
        IBroadsideCampaignsMinimal(campaignsContract).activateCampaign(campaignId);
    ...
    //   vote runs in BroadsideGovernance. Mute is collateralised — if the
    ...
    function mute(uint256 campaignId) external payable nonReentrant whenNotFrozen {
        ...
        (uint8 status,,) = IBroadsideCampaignsMinimal(campaignsContract)
        ...
        // getter.
        try IBroadsideCampaignsForMute(campaignsContract).getCampaignAdvertiser(campaignId) returns (address adv) {
    ...
    function _migrate(address oldContract) internal override {
        BroadsideActivationBonds old = BroadsideActivationBonds(payable(oldContract));
