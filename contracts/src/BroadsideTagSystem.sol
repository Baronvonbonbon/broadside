...
import "./interfaces/IBroadsideTagRegistry.sol";
import "./interfaces/IBroadsideTagCurator.sol";
import "./interfaces/IBroadsidePauseRegistry.sol";

/// @title  BroadsideTagSystem
/// @notice Unified tag plumbing carved out of BroadsideCampaigns for mainnet
///         EIP-170. Owns:
...
///             from BroadsideCampaigns._createCampaign with the campaign's
...
///
///         Consumers:
///           - BroadsideCampaignAllowlist reads `hasAllTags` during
///             `_validateAndSeat` to check tag match per allowlist-add.
///           - BroadsideClaimValidator reads `hasAllRequiredTags` per claim
///             in ALLOWLIST/OPEN gate logic.
contract BroadsideTagSystem is IBroadsideTagSystem, BroadsideUpgradable {
    ...
    IBroadsideTagRegistry public tagRegistry;       // stake-gated tag governance
    ...
    function setTagRegistry(address registry) external onlyOwner {
        ...
        tagRegistry = IBroadsideTagRegistry(registry);
    ...
    function _isTagApproved(bytes32 tag) internal view returns (bool) {
        ...
        if (tagCurator != address(0)) {
            try IBroadsideTagCurator(tagCurator).isTagApproved(tag) returns (bool ok) {
                return ok;
            } catch {
                return false;
            }
    ...
    function setPublisherTags(bytes32[] calldata tagHashes) external whenNotFrozen {
        ...
        IBroadsidePublishers.Publisher memory pub = publishers.getPublisher(msg.sender);
    ...
    /// @notice Backwards-compat alias matching the historic BroadsideCampaigns surface.
    ...
    /// @inheritdoc IBroadsideTagSystem
    ...
    function setCampaignTagMode(uint256 campaignId, uint8 mode) external whenNotFrozen {
        ...
        // gate is performed via the bytes here -- the IBroadsideCampaigns
    ...
    function _migrate(address oldContract) internal override {
        BroadsideTagSystem old = BroadsideTagSystem(oldContract);
...
/// @dev Inline read interface to avoid pulling the full IBroadsideCampaigns
