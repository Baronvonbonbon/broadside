...
contract BroadsideZKStake is BroadsideUpgradable, ReentrancyGuard {
    ...
    function _migrate(address oldContract) internal override {
        BroadsideZKStake old = BroadsideZKStake(oldContract);
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        require(BroadsideZKStake(successor).token() == token, "token-mismatch");
