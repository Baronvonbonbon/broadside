...
import "./interfaces/IBroadsideCampaignsMinimal.sol";
...
interface IBroadsideUpgradable_Router {
    function freeze() external;
    function migrate(address oldContract) external;
}

/// @title BroadsideRouter
...
///
///         Governance ladder:
///           Phase 0 (Admin)   — BroadsideAdminGovernance: team multisig direct approval
///           Phase 1 (Council) — BroadsideCouncil: N-of-M trusted council voting
///           Phase 2 (OpenGov) — BroadsideGovernance: full conviction-weighted open governance
///
///         GovernanceV2 (Phase 2) compatibility:
///           Set govV2.campaigns = router  (Router implements IBroadsideCampaignsMinimal)
...
contract BroadsideRouter is BroadsideOwnable, PaseoSafeSender {
    ...
    ///         OpenGov (`BroadsideGovernance`, which has no upgrade/regression call
    ...
    IBroadsideCampaignsMinimal public campaigns;
    IBroadsideCampaignLifecycle public lifecycle;
    ...
    constructor(
        ...
        campaigns = IBroadsideCampaignsMinimal(_campaigns);
        lifecycle = IBroadsideCampaignLifecycle(_lifecycle);
        governor = _governor;
    ...
    function setCampaigns(address addr) external onlyOwner {
        ...
        campaigns = IBroadsideCampaignsMinimal(addr);
    }
    ...
    function setLifecycle(address addr) external onlyOwner {
        ...
        lifecycle = IBroadsideCampaignLifecycle(addr);
    }
    ...
    ///         through the IBroadsideRouter_Upgradable modifier.
    ...
    // Merged from BroadsideAdminGovernance. Gated on `phase == Admin` so the owner
    ...
    // -------------------------------------------------------------------------
    // IBroadsideCampaignsMinimal passthrough
    // Used by BroadsideGovernance when govV2.campaigns = address(router)
    // -------------------------------------------------------------------------
    ...
    ///         or don't implement BroadsideUpgradable; operators alert on this
