...
/// @title  BroadsideAdvertiserRegistry
/// @notice Standalone advertiser-side registry — the symmetric counterpart of
///         BroadsidePublishers (relaySigner + profileHash + rotation cooldown), so
///         the advertiser half of dual-sig settlement has its own contract
///         instead of living inside the already-EIP-170-tight BroadsideCampaigns.
///
...
///
/// @dev    Migration model (BroadsideUpgradable): this is a NEW contract — a pure
///         addition. Deploy → setRouter → wire readers; no state migration.
///         BroadsideDualSig can be pointed here for advertiser-relay-signer
///         resolution in a later coordinated upgrade (it currently reads
///         BroadsideCampaigns.getAdvertiserRelaySigner); until then this registry
///         drives off-chain endpoint discovery for relays. The mappings/getters
///         intentionally match BroadsideCampaigns' advertiser API so a settlement
///         read can switch sources without an interface change.
contract BroadsideAdvertiserRegistry is BroadsideUpgradable {
    ...
    ///         BroadsidePublishers.RELAY_SIGNER_ROTATION_COOLDOWN — a key briefly
    ...
    ///         one tx (mirror of BroadsidePublishers.setRelaySignerAndProfile).
    ...
    // ── Readers (match the BroadsideCampaigns advertiser API for drop-in reads) ──
    ...
    ///      Called by BroadsideUpgradable.migrate (governance-gated, old must be frozen,
    ...
    function _migrate(address oldContract) internal override {
        BroadsideAdvertiserRegistry old = BroadsideAdvertiserRegistry(oldContract);
