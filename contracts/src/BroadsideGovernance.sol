...
///
///         Alpha-2 changes:
///           - Termination delegates to BroadsideCampaignLifecycle (not Campaigns directly).
...
contract BroadsideGovernance is PaseoSafeSender, BroadsidePlumbingLockable, ParameterRetuneGuard {
    ...
    function vote(uint256 campaignId, bool aye, uint8 conviction) external payable nonReentrant whenNotFrozen {
        ...
        (uint8 status,,) = IBroadsideCampaignsMinimal(campaigns).getCampaignForSettlement(campaignId);
    ...
    function evaluateCampaign(uint256 campaignId) external {
        ...
        if (status == 0) {
            ...
            if (ayeWins) {
                ...
                IBroadsideCampaignsMinimal(campaigns).activateCampaign(campaignId);
    ...
    function _migrate(address oldContract) internal override {
        BroadsideGovernance old = BroadsideGovernance(payable(oldContract));
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideGovernance(payable(successor)).acceptMigration{value: bal}();
    }
