...
import "./interfaces/IBroadsideRelayGovernance.sol";
import "./interfaces/IBroadsidePauseRegistry.sol";

/// @title  BroadsideRelayGovernance
/// @notice G-1 first close: conviction-vote fraud proposals against relays.
///         Symmetric to BroadsidePublisherGovernance / BroadsideAdvertiserGovernance.
...
///             (phase-gated on OpenGov via BroadsideUpgradable.whenOpenGovPhase).
contract BroadsideRelayGovernance is
    IBroadsideRelayGovernance,
    PaseoSafeSender,
    BroadsideUpgradable,
    ParameterRetuneGuard
{
    ...
    /// @notice Accept slashed funds from BroadsideRelayStake.slash(). The receive
    ...
    function _migrate(address oldContract) internal override {
        BroadsideRelayGovernance old = BroadsideRelayGovernance(payable(oldContract));
        quorum = old.quorum();
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsideRelayGovernance(payable(successor)).acceptMigration{value: bal}();
    }
