...
/// @title  BroadsideNullifierRegistry
...
contract BroadsideNullifierRegistry is IBroadsideNullifierRegistry, BroadsideUpgradable {
    ...
    /// @inheritdoc IBroadsideNullifierRegistry
    ...
    function _migrate(address oldContract) internal override {
        nullifierWindowBlocks = BroadsideNullifierRegistry(oldContract).nullifierWindowBlocks();
    }
    ...
    function _usedAnywhere(uint256 campaignId, bytes32 nullifier) internal view returns (bool) {
        ...
        if (pred != address(0)) {
            return BroadsideNullifierRegistry(pred).isNullifierUsed(campaignId, nullifier);
        }
        return false;
    }
}
