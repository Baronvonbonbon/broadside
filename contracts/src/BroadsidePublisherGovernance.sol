...
import "./interfaces/IBroadsidePublisherGovernance.sol";
...
/// @title BroadsidePublisherGovernance
...
///
///         On fraud upheld:
///           - slashBps of publisher's stake is slashed via BroadsidePublisherStake.
...
contract BroadsidePublisherGovernance is
    IBroadsidePublisherGovernance,
    PaseoSafeSender,
    BroadsidePlumbingLockable,
    ParameterRetuneGuard
{
    ...
    IBroadsidePublisherStake public publisherStake;
    ...
    // bond and submits an evidence CID. The Council (via BroadsideCouncil → propose
    ...
    constructor(
        ...
        publisherStake = IBroadsidePublisherStake(_publisherStake);
        challengeBonds = _challengeBonds == address(0) ? IBroadsideChallengeBonds(address(0)) : IBroadsideChallengeBonds(_challengeBonds);
    ...
    function setConvictionCurve(uint256 a, uint256 b) external onlyOwner {
        // Mirrors the AUDIT-PASS-5 L6 guard in BroadsideGovernance: keep the
    ...
    function setPublisherStake(address addr) external onlyOwner whenPlumbingUnlocked {
        ...
        publisherStake = IBroadsidePublisherStake(addr);
    }
    ...
    /// @inheritdoc IBroadsidePublisherGovernance
    ...
    function _migrate(address oldContract) internal override {
        BroadsidePublisherGovernance old = BroadsidePublisherGovernance(payable(oldContract));
        quorum = old.quorum();
    ...
    function migrateFundsTo(address successor) external onlyGovernance nonReentrant {
        ...
        if (bal > 0) BroadsidePublisherGovernance(payable(successor)).acceptMigration{value: bal}();
    }
